 ...
 * 流式布局 - 实验中
 */
public class AutoSizingFlowLayout extends BaseViewGroup {

  /**
   * 存储所有的View，按行记录
   */
  private List<List<TagView>> mLineList = new ArrayList<>();
  /**
   * 记录每一行的最大高度
   */
  private List<Integer> mLineHeightList = new ArrayList<>();

  public AutoSizingFlowLayout(Context context) {
    super(context);
  }

  public AutoSizingFlowLayout(Context context, AttributeSet attrs) {
    super(context, attrs);
  }

  public AutoSizingFlowLayout(Context context, AttributeSet attrs, int defStyleAttr) {
  ...
  @Override protected void onLayout(boolean changed, int l, int t, int r, int b) {
    ...
    int lineHeight = 0;
    // 存储每一行所有的childView
    List<TagView> line = new ArrayList<>();
